...
TEST(analysis, example1)
{
    ...
    const auto analysis = analyze(rev, &code[0], 0, code.size());
...
TEST(analysis, jumpdest_skip)
{
    ...
    auto analysis = evmone::analyze(rev, &code[0], 0, code.size());
...
TEST(analysis, example1_eof1)
{
    ...
    const auto header = read_valid_eof1_header(code.data());
    const auto analysis = analyze(rev, &code[0], header.code_begin(), header.code_end());
